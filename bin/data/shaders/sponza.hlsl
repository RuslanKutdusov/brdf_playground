...
PSOutput ps_main(VSOutput input, float4 pixelPos : SV_Position)
{
	...
	float3 baseColor = BaseColorTexture.Sample(LinearWrapSampler, input.uv).rgb;
	...
	MaterialData materialData = InitMaterialData(kMaterialTexture, metalness, roughness, 1.0f, baseColor);
	...
	{
		// Lo = ( Fd + Fs ) * (n,l) * E
		output.directLight.rgb = CalcDirectLight(normal, LightDir.xyz, view, materialData) * LightIlluminance.rgb;
		if (EnableShadow)
		...
		// Lo = ( Fd + Fs ) * (n,l) * E
		output.directLight.rgb += CalcDirectLight(normal, lightDir, view, materialData) * irradiance.rgb;
	}
	if (EnableEnvEmitter)
	{
